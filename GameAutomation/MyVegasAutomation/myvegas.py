...
# into terminal type in "adb devices"
...
def restart():
    print('restart')
    device.shell('am force-stop com.playstudios.myvegas')
    time.sleep(5)
    device.shell('am start -n com.playstudios.myvegas/.MyVegasActivity')
    time.sleep(5)
...
def comparison():
    ...
    for x in range(numberofpictures):
        needle_img = cv2.imread('grayed/' + str(x + 1) + '.png', cv2.IMREAD_UNCHANGED)
...
def convert():
    ...
    numberofpictures = len(os.listdir('templates'))
    try:
        os.mkdir('grayed')
    except OSError:
        pass
    for x in range(numberofpictures):
        needle_img = cv2.imread('templates/' + str(x + 1) + '.png', cv2.IMREAD_UNCHANGED)
        gray_Needle = cv2.cvtColor(needle_img, cv2.COLOR_BGR2GRAY)
        cv2.imwrite('grayed/' + str(x + 1) + '.png', gray_Needle)
...
while True:
    counter += 1
    try:
        activeApp = (device.shell("dumpsys window | grep -E 'mCurrentFocus'"))
    except RuntimeError:
        print('Reconnecting')
        deviceconnection()
    if counter >= 1500:
